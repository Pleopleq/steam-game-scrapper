...
def parse(data):
    ...
    for game in games:
        ...
        price = game.find('div', {'class': 'search_price'}).text.strip().split('Mex$')[1]
        images = game.findAll('img', attrs = {'srcset' : True })
        try:
            discprice = game.find('div', {'class': 'search_price'}).text.strip().split('Mex$')[2]
        except:
            discprice = price
...
results = []
for x in range(0, totalresults(url), 1000):
